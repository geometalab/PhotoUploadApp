...
class NearbyCategoriesService{
  ...
  Future<List<CategoryLocation>> getNearbyCategories (double lat, double lng, int radius) async {
    ...
    if(response.statusCode == 200){
      ...
      for(int i = 0; i < xmlList.length; i++){   // Very sloppy way of extracting from xml by searching for keys in a string and then using substring to extract data
        ...
        var coordinateString = resultString.substring( coordinateStartIndex + 6, coordinateEndIndex);
  ...
  Future<List<Marker>> markerBuilder (Future<List<CategoryLocation>> locationList) async {
